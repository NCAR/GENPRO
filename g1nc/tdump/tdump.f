...
C    * * * * * * * * * * * * * * * * * * * * * * * * *
C
C    Revision 5:  961008  RLR  (fixed GENPRO-1 header read format stmt #9045
C                                in SUBROUTINE G1HEDR)
C    Revision 4:  940311  RLR  (changed NRATES to 17 and added rate 310;
C                               changed read format to handle more rates)
C    Revision 3:  930813  RLR  (fix header record counter)
C    Revision 2:  921030  RLR  (set parameters for number of rates
C                               and number of variables)
C
...
C      F.  NO. OF LOGICAL RECORDS PER PHYSICAL RECORD.
C
C
...
C
      IMPLICIT NONE
C
C  Define size of variable array and the number of rates
      INTEGER    VARSIZE,     NRATES
      PARAMETER (VARSIZE=300, NRATES=17)
C
C  Note:  NRATES defines the number of known rates.  Line # 1461 defines
C           the list.
C
C---Common areas:
      COMMON/READIT/IRWFLG,IARRAY(4000),IDATA(10000)
      COMMON/INPARMS/IUNIT, LIBVER, GNPVER, IHOWTO, KPRT,IMRATE(NRATES),
...
C
C---Common areas:
      COMMON/HDINFO1/LOGBIT,NOLOG,NOVAR,TERM(VARSIZE),FACTOR(VARSIZE),
     $               NBITS(VARSIZE),NFSTBT(VARSIZE),NRATE(VARSIZE),KR
      COMMON/HDINFO2/NAMVAR(VARSIZE)
      COMMON/TDUMP1/DATLOG,IWDSZ
      COMMON/TDUMP2/PROJCT(5), UNITS(VARSIZE)
      COMMON/PNRTR1/INUM,LOCPOS(VARSIZE),LOCMIN(VARSIZE),
     $     NFSTWD(VARSIZE),MINUTE(60,4000),BEGREC(60),TIME(60,3),NUMSEC
      COMMON/PNRTR2/NAMDMP(VARSIZE)
      COMMON/HDPRT1/MON,DAY,YRR,HR,MIN,SEC,BGSNP(3),
     $                  EDSNP(3),MCHINE(1),TITLE(VARSIZE,5)
...
C---Local area variable declarations:
      INTEGER   IARRAY(VARSIZE),NDUMMY(10),KARRAY(4000),JARRAY(5000)
...
C9045       FORMAT(5X,I2,11X,I3,12X,I4,23X,I2)
C  Fix to handle the SANDHILLS project, 1980-489A (Should be applicable to all)
 9045       FORMAT(4X,I3,10X,I4,11X,I5,22X,I3)
C
            HDRLOG=LOGBIT+11
            HDRSIZ=HDRLOG*8*104
            LOGBIT=LOGBIT*IWRDS
...
C
     ...
     $                NBITS(VARSIZE),NFSTBT(VARSIZE),NRATE(VARSIZE),KR
      ...
      COMMON/TDUMP2/PROJCT(5), UNITS(VARSIZE)
      COMMON/PNRTR1/INUM,LOCPOS(VARSIZE),LOCMIN(VARSIZE),
     1  NFSTWD(VARSIZE),MINUTE(60,4000),BEGREC(60),TIME(60,3),NUMSEC
      COMMON/PNRTR2/NAMDMP(VARSIZE)
C
C---Common area variable definitions:
      INTEGER     LOGBIT, NOLOG, NOVAR, NBITS, NFSTBT, NRATE, KR
...
C---Read the rates to be printed, only significant if dumping by rate.
      READ(5,9900) DUMMY
      READ(5,9900) DUMMY
      READ(5,9930) (IMRATE(I),I=1,NRATES)
C
...
C---Local variables:
      ...
      INTEGER IPRPOS(10), NAMPOS(10), INRATE(NRATES), IRRPOS(10)
      REAL    SEC, SECD
C
      DATA INRATE
