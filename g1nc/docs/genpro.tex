...
\begin{itemize}[noitemsep]
	...
	\item \texttt{time\_coordinate} -- Set to 
	...
	\item \texttt{geospatial\_vertical\_min}, \texttt{geospatial\_vertical\_max} -- 
	\item \texttt{geospatial\_vertical\_units} -- 
	...
	\item \texttt{ProcessorRevision} -- Automatically set to the branch/
	\item \texttt{Categories} -- Set to ``\texttt{Position,Thermodynamic,Aircraft State,Atmos. State}"
	\item \texttt{geospatial\_vertical\_positive} -- 
	\item \texttt{wind\_field} -- Set to ``\texttt{WSPD WDRCTN WI}".
	\item \texttt{}
\end{itemize}
