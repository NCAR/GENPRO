...
%	      GENPRO-1 file.
	...
	\item \texttt{Address}
	\item \texttt{creator\_url} -- 
	\item \texttt{ConventionsURL} --
	\item \texttt{Platform} --
	\item \texttt{time\_coverage\_start}, \texttt{time\_coverage\_end} -- 
	\item \texttt{TimeInterval} -- 
	\item \texttt{geospatial\_lat\_min}, \texttt{geospatial\_lat\_max}, \texttt{geospatial\_lon\_min}, \texttt{geospatial\_lon\_max} -- 
\end{itemize}

Every variable has three attributes:
\begin{itemize}
	\item \texttt{long\_name} -- a human readable description of the variable,
