...
C     Output: unit 90 -- ADSRAW job deck                                        
C             unit 92 -- APPEND file, to be appended to Adsraw deck             
C                                                                               
...
C---------adjust lindex to the end of the string arg2
      fulpth(1:lindex) = arg2(1:lindex)
      fulpth(lindex+1:lindex+8) = '/adsraw.'
      fulpth(lindex+9:lindex+14) = 'append'
...
C write out 1st 2 JCL statements                                                
      WRITE(90,'('' JOB,JN=RAWDMP,US='',A4,                                   
     $''41113'',A3,'',T=59,CL=EX1,OLM=9999,*MS.'',/,                        
...
C is completed and the Append file will exist as flag for calling program       
      WRITE(92,'('' ADSRAW,'',A8,'',YEAR=19'',I2,'','',A3,'','',A5,         
     $  '',NFILES=1,RAWDUMP,TAPE='',A6,''.'')')                                 
     $  USER,YEAR,PROJNO,FLTNO,TAPENO                                           
      WRITE(92,'('' EXIT.'',/,'' DUMPJOB.'',/,'' DEBUG.'')')          
  500 continue                                                                  
      RETURN                                                                    
      END                                                                       
